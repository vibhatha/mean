...
exports.update = function(req, res) {
    ...
    power.smartplugs=(req.body.smartplugs._id);
...
exports.powerByDevice = function(req, res, next, devices) {
	// Use the model 'findById' method to find a single article 
    console.log('Power By Device');
	Power.find({filter:{where:{devices:devices}}}).sort('-created').populate('creator', 'firstName lastName fullName').exec(function(err, powers) {
		if (err) {
			// If an error occurs send the error message
			return res.status(400).send({
				message: getErrorMessage(err)
